        ...
        <div v-if="!selectedDomain">
          <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4">
            <div>
              ...
              <p class="text-sm text-light-text-secondary dark:text-dark-text-secondary mt-1">{{ $t('domains_found', Object.keys(grouped).length) }}</p>
            </div>
            <div class="flex items-center gap-2 relative" ref="exportMenuContainer">
          ...
          <transition-group :tag="layout === 'grid' ? 'div' : 'ul'" :class="layoutWrapperClass" name="layout-item">
            <li v-for="(emails, domain) in sortedDomains" :key="domain" :class="layoutItemClass">
              ...
              <div v-else @click="selectDomain(domain)" class="cursor-pointer p-5 flex flex-col h-full">
                ...
                <ul class="space-y-1 overflow-hidden flex-grow">
                  <li v-for="email in emails.slice(0, 3)" :key="email" class="text-sm text-light-text-secondary dark:text-dark-text-secondary truncate">{{ email }}</li>
                </ul>
...
// 2. SETUP DE STORES E UTILS
const auth = useAuthStore();
const { t } = useI18n();

// 3. DECLARAÇÃO DE TODO O ESTADO REATIVO
